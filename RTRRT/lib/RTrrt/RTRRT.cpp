...
void RTRRTplanner::ExpandRewiring(long RunTime)
{
    ...
    while (utime_ns() - t0 < RunTime)
    {
        ...
        }
        {
            ...
            Neighbor_search search_Closest(NeighborSearchTree, {rand_x, rand_y}, KnearN); //@@@
            ...
            {
                if (sqrtf32(search_Closest.begin()->second) > RnearR)
                {
        ...
        while (Qs.size() && (utime_ns() - t0 < RunTime))
        {
            ...
            Qs.pop();
            Neighbor_search search_near(NeighborSearchTree, {x_s.x, x_s.y}, KnearN); //@@@
            for (const Point_with_distance pwd : search_near)
            {
                RTpoint x_near = {(float)pwd.first.x(), (float)pwd.first.y()};
                RTrrt_Cost c_old = Node_Map[x_near].cost;
...
RTrrt_Cost RTRRTplanner::GetPath(std::list<RTpoint> &path)
{
    c_base = -1;
    path.clear();
    if (goal.x <= 0)
        return {1, 3.4e38};
    if(!CrashDetection(goal, root))
    {
        path.push_back(root);
        path.push_back(goal);
        return {0, root%goal};
    ...
    else
    {
        ...
        while (Qpk.size())
        {
            ...
            RTrrt_Cost c_now = {Node_Map[pk_now.second].cost.collision, pk_now.second % goal};
            if (c_now < c_min)
            {
                c_min = c_now;
                x_min = pk_now.second;
