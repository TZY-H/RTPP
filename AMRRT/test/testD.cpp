...
std::map<std::string, std::string> taskdiffusionMatrixs;
std::map<std::string, std::string> taskindex2Poss;
...
void InitTaskMapD(void)
{
    ...
    taskmap["MAZE0"] = {{1500, 1850}, {{200, 200}, {3100, 200}, {3100, 3100}, {200, 3100}}, {2250.927, 2406.131, 2967.605, 1931.922}, "/home/tzyr/WorkSpace/CDRT2/map/MAZE0.png", {0.1, 1.1, 28}};   // 0.1,1.1
    ...
    taskdiffusionMatrixs["FLOOR5"] = "../diffusion/diffusion_matrix/FLOOR5_diffusion_matrix.json";
    taskdiffusionMatrixs["GAME0"] = "../diffusion/diffusion_matrix/GAME0_diffusion_matrix.json";
    taskdiffusionMatrixs["MAZE0"] = "../diffusion/diffusion_matrix/MAZE0_diffusion_matrix.json";
    taskdiffusionMatrixs["MESS6"] = "../diffusion/diffusion_matrix/MESS6_diffusion_matrix.json";

    taskindex2Poss["FLOOR5"] = "../diffusion/index_to_pos/FLOOR5_index_to_pos.json";
    taskindex2Poss["GAME0"] = "../diffusion/index_to_pos/GAME0_index_to_pos.json";
    taskindex2Poss["MAZE0"] = "../diffusion/index_to_pos/MAZE0_index_to_pos.json";
    taskindex2Poss["MESS6"] = "../diffusion/index_to_pos/MESS6_index_to_pos.json";
...
int main(void)
{
    ...
    for (auto &pair : taskmap)
    {
        ...
        {
            AMRRTplanner simPlanner;
            simPlanner.InitializeInvironment(task.Path.data(), 128);
            simPlanner.InitializeDiffusion(taskdiffusionMatrixs[name].data(),
                                           taskindex2Poss[name].data(), 200.0 / simPlanner.shapeX);
            ...
            for (;;)
            {
                ...
                if (agent_tag % task.G[i] <= 50.0 || key == '0')
                    break;
            }
