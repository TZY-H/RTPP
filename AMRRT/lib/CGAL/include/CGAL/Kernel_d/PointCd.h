...
//
// $URL: https://github.com/CGAL/cgal/blob/v5.6/Kernel_d/include/CGAL/Kernel_d/PointCd.h $
...
#define PointCd PointCd2
  ...
  public:
    ...
    PointCd(int x, int y, int z, int w) : Base(Tuple(FT(x), FT(y), FT(z), MatchHelper()))
    {
      CGAL_assertion_msg(w != 0, "PointCd::construction: w == 0.");
      vector_rep() /= w;
    }
    ...
    friend std::istream &operator>> <>(std::istream &, PointCd<FT, LA> &);
    friend std::ostream &operator<< <>(std::ostream &, const PointCd<FT, LA> &);
